...
var App = function () {
    ...
    var handleSidebarMenu = function () {
        jQuery('.page-sidebar').on('click', 'li > a', function (e) {
            ...
            if (sub.is(":visible")) {
            	...
            	var reqUrl = jQuery('#resUrl',jQuery(this)).val();
            	var tarMenu = jQuery('.sub-menu',jQuery(this).parent());
            	var resId =  jQuery('#resId',jQuery(this)).val();
            	
            	if(!tarMenu.html()){//add by leaf
            		loadSideBarSubMenu(reqUrl,tarMenu,resId);
            	}
    ...
    //ajax加载导航条子目录 -- add by leaf
    var loadSideBarSubMenu = function (url,subMenu,resId){
    	if(!subMenu || !url || !resId){
    		return;
    	}
        ...
        $.ajax({
            ...
            success: function (objJson) {
            	if(objJson){
            		var data = objJson.subMenus;
            		for(var i=0;i<data.length;i++){
            			var str="";
            			if(data[i].leaf_flag == 1){
            				str += "<li><a href=\""+context_+"/"+data[i].res_url+"\">";
            			}else{
            				str += "<li><a href=\"javascript:;\">";
            			}
            			
            			str += "<i class=\"fa fa-leaf\"></i>";
            			str += "<span class=\"title\">"+data[i].label+"</span>";
            			if(data[i].leaf_flag == 0){
            				str += "<span class=\"arrow\"></span>";
            			}
            			str +="<input id=\"resId\" type=\"hidden\" value=\""+data[i].res_id+"\">";
            			str +="<input id=\"resUrl\" type=\"hidden\" value=\""+data[i].res_url+"\">";
            			str += "</a>";
            			if(data[i].leaf_flag == 0){
            				str += "<ul class=\"sub-menu\"></ul>";
            			}
            			str += "</li>";
            			subMenu.append(str);
            		}
